...
(function (window, document) {
  var factory = function ($, ColumnFilter) {
    ...
    ColumnFilter.filter.numberRangeBase = $.extend(true, {}, ColumnFilter.filter.numberRange);
    ColumnFilter.filter.numberRange = {};
    $.extend(ColumnFilter.filter.numberRange, ColumnFilter.filter.numberRangeBase, {
      separator: '~',
      dom: function(th) {
          var self = this, input1, input2;

          this.div = $('<div>').addClass('input-group input-numberrange');
          input1 = $('<input>', {
            type: 'number'
          }).addClass('form-control input-sm').attr('name', 'start').appendTo(this.div);
