    ...
    function MyLeadslistController($http, $resource,  $scope, toaster, CatalogEnquiry,  grouptype, Company, BuyerList, Buyers, Country, ngDialog,  DTOptionsBuilder, DTColumnDefBuilder, DTColumnBuilder, $compile, $state, CheckAuthenticated, Upload, resendsupplier, $cookies, $localStorage) {
     ...
     /*   vm.dtColumnDefs = [
        ...
        vm.dtColumnDefs = [
            ...
            DTColumnDefBuilder.newColumnDef(4).withTitle('State').notVisible(),
            DTColumnDefBuilder.newColumnDef(5).withTitle('City').notVisible(),
            ...
            DTColumnDefBuilder.newColumnDef(12).withTitle('Action').notSortable()//.withOption('width', '25%')
                .renderWith(function(data, type, full, meta) {
                    var htmlbutton = ''
                        ...
                        if(full[11] != 'Resolved') {
                            htmlbutton += '<div><button type="button" ng-click="Resolve(' + full[0] +')" class="linkButton" >Resolve</button></div>';
                        }
                        ...
                        if(full[1]['applogic_conversation_id'] != null){
                            htmlbutton += '<div class="linkButton"  ng-click="openTopicBasedChat(\''+full[1]['buying_company_chat_user']+'\','+full[1]['applogic_conversation_id']+')" data-mck-id="'+full[1]['buying_company_chat_user']+'" data-mck-name="Buyer" data-mck-topicid="'+full[1]['applogic_conversation_id']+'">Chat</div>';
                        }
