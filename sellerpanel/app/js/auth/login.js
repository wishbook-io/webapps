...
(function() {
    ...
    LoginFormController.$inject = ['$http', '$state', '$scope', '$rootScope', 'CheckAuthenticated', 'djangoAuth', 'Country', 'Users', '$location', 'ToDashboard', 'Company', 'toaster', '$stateParams', '$cookies', 'ngDialog', '$localStorage'];
    function LoginFormController($http, $state, $scope, $rootScope, CheckAuthenticated, djangoAuth, Country, Users, $location, ToDashboard, Company, toaster, $stateParams, $cookies, ngDialog, $localStorage) {
              ...
              .then(function(response) {
                  djangoAuth.profile().then(function(data){
                        ...
                        if(data.is_staff == true){
                            ...
                            alert("Super admin can login to https://admin.wishbook.io");
                            djangoAuth.logout();
                            //$rootScope.logout();
                            //return;
                            //$state.go('app.companies');
                        }
                        else if(data.companyuser.company_group_flag.wholesaler_distributor == true || data.companyuser.company_group_flag.manufacturer == true){
                          localStorage.removeItem('is_staff');
                          //$state.go('app.browse');
                          //$state.go('app.catalog');
                            $state.go('app.dashboard');
                        }
                        else if(data.companyuser.company_group_flag.retailer == true || data.companyuser.company_group_flag.online_retailer_reseller == true){
                          alert("Retailer can not login to this panel, Only Seller can login to this panel.");
                          djangoAuth.logout();
                          //return;
                        }
