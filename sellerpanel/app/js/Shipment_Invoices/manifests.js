(function() {
    ...
    manifestsController.$inject = ['$window', '$http', '$resource',  '$scope', '$stateParams', 'Manifest', 'ManifestCopy', 'DropshipOrderInvoice', 'Shipments', 'SalesOrders', 'PurchaseOrders', 'OrderInvoice', 'MultiOrder', 'Company',  'ngDialog', 'toaster', 'DTOptionsBuilder', 'DTColumnDefBuilder', 'DTColumnBuilder', '$compile', '$state', 'CheckAuthenticated', '$cookies', '$localStorage', 'Upload'];
    function manifestsController($window, $http, $resource,  $scope, $stateParams, Manifest, ManifestCopy, DropshipOrderInvoice, Shipments, SalesOrders, PurchaseOrders, OrderInvoice, MultiOrder, Company,  ngDialog, toaster, DTOptionsBuilder, DTColumnDefBuilder, DTColumnBuilder, $compile, $state, CheckAuthenticated, $cookies, $localStorage, Upload) {
        ...
        $scope.openUploadAttachments = function (shipmentid, filetype)
        {
            ...
            Manifest.get(params,function (result) {
              $scope.manifest_data = result;
              // $scope.manifest_data.shipping_provider = result.shipping_provider;
              // $scope.manifest_data.shipping_service = result.shipping_service;
              // $scope.manifest_data.shipments = result.shipments;
...
*/
            ...
            //var params = { 'id': $scope.shipment_id, 'cid': $scope.company_id };
            var params = $scope.manifest_data;
            params['cid'] = $scope.company_id;
            delete params['shipments'];
            if ($scope.ackCopyImage) {
                ...
                params['signed_copy'] = acknowledgement_copy_image;

            }
            else if ($scope.ackCopypdffile) {
                params['signed_copy'] = $scope.ackCopypdffile;
            }
            //param['sub_resource'] = 'signed-copy';
            console.log(params);
            $(".modelform3").addClass(progressLoader());

            ManifestCopy.put(params).$promise.then(function (result)
            {
                $(".modelform3").removeClass(progressLoader());
                vm.successtoaster = {
                    type: 'success',
                ...
                reloadData();
                    ...
                    vm.successtoaster = {
                        type: 'error',
                        title: 'Success',
                        text: 'Manifest couldnt upload because of'+ error
                    };
                }
            );
        ...
        vm.dtOptions = DTOptionsBuilder.newOptions()
                        ...
                        .withLightColumnFilter({
                            ...
                            2 : { "type":  "text"},
                            3 : { "type": "text" },
                            4 : { "type": "select", values: [{ "value": "CLOSED", "label": "CLOSED" }, { "value": "CREATED", "label": "CREATED" }] },
                            5 : { "type" : "dateRange"},
                        })
        ...
        vm.dtColumnDefs = [
            ...
            DTColumnDefBuilder.newColumnDef(2).withTitle('Shipping Provider'),
            DTColumnDefBuilder.newColumnDef(3).withTitle('Provider Service'),
            DTColumnDefBuilder.newColumnDef(4).withTitle('Status').notSortable(),
            DTColumnDefBuilder.newColumnDef(5).withTitle('Created Date').notSortable(),
            ...
            .renderWith(function(data, type, full, meta) {
                ...
                if (full[4] === 'CLOSED')
                {
                  htmlbutton += '<div><button type="button" onclick="window.open(\''+ full[1]['signed_copy'] +'\')" class="linkButton">View Manifest</button></div>'
                }
                if (full[4] === 'CREATED')
                {
                ...
                htmlbutton += '<div><button type="button" class="linkButton"><a class="linkButton" target="_blank" href="/#/pages/printmanifest/?id='+full[0]+'">Print Manifest</a></button></div>'
