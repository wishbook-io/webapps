...
(function() {
    ...
    CreatePurchaseOrderController.$inject = ['$resource', '$filter', '$scope', 'CatalogsAllSupplier', 'Catalog', 'SupplierList', 'Company', 'PurchaseOrders', 'OrderInvoice', 'ngDialog', 'toaster', 'DTOptionsBuilder', 'DTColumnDefBuilder', 'DTColumnBuilder', '$compile', '$state', 'CheckAuthenticated', '$cookies', '$localStorage', 'sharedProperties'];
    function CreatePurchaseOrderController($resource, $filter, $scope, CatalogsAllSupplier, Catalog, SupplierList, Company, PurchaseOrders, OrderInvoice, ngDialog, toaster, DTOptionsBuilder, DTColumnDefBuilder, DTColumnBuilder, $compile, $state, CheckAuthenticated, $cookies, $localStorage, sharedProperties) {
        ...
        $scope.single_piece_avl = false;
        $scope.cdata.sell_full_catalog == 'false';
  ...
  /*      if($state.current.name == 'app.publiccatalog'){
          ...
          $scope.catalogname =  $scope.catalog_name;
          vm.catalogs_all_suppliers = CatalogsAllSupplier.query({'id': $scope.catalogid},function(success){
            console.log(success);
          });
        }
        else{
         ...
         if($state.current.name == 'app.receivedcatalog'){
          $scope.receivedcatalogflag = true;
          vm.order.supplier = $scope.catalog_supplier;
          vm.order.supplier_name = $scope.catalog_supplier_name;
         }
         else{
          $scope.receivedcatalogflag = false;
         }

        
        $scope.cdata.catalog = $scope.catalogid;
        Catalog.get({'id': $scope.catalogid, 'cid': $scope.company_id},function(success){
          $(".modelform-order").addClass(progressLoader());
          if(!$scope.receivedcatalogflag){
            if(success.sell_full_catalog == true){
              $scope.single_piece_avl = false;
               $scope.cdata.sell_full_catalog = 'true';
              $scope.getSupplierList();
            }
            else{
              $scope.single_piece_avl = true; 
              $scope.cdata.sell_full_catalog = 'false';
              $scope.getSupplierList();
            }
          }
      ...
      //  vm.agents = Company.query({'id':$scope.company_id, 'sub_resource':'brokers'});
        ...
        $scope.CatalogArray = function() {
            ...
            console.log($scope.cdata.catalog);
            console.log($scope.cdata.sets);
            console.log($scope.cdata.sell_full_catalog);
            console.log(vm.order.supplier);
           // if(($scope.cdata.catalog && $scope.cdata.sets && $scope.cdata.sell_full_catalog ) || ($scope.cdata.catalog && $scope.cdata.sets && $scope.receivedcatalogflag == true )){
              if(($scope.cdata.catalog && $scope.cdata.sets && $scope.cdata.sell_full_catalog && vm.order.supplier) || ($scope.cdata.catalog && $scope.cdata.sets && $scope.receivedcatalogflag == true  && vm.order.supplier)){
                ...
                Catalog.get({"id":$scope.cdata.catalog, "cid":$scope.company_id},
                   function (result){
                     ...
                     // catalog.packingtype = $scope.cdata.packingtype;
                      ...
                      catalog.sell_full_catalog = result.sell_full_catalog;
                      catalog.selectall = true;
                      console.log(catalog.sell_full_catalog);
       ...
       // $scope.selectall = true;
        $scope.toggleAll = function(index){
          console.log(index);
          console.log($scope.catalogstobeorder[index].selectall);
          //$scope.catalogstobeorder[index].selectall = !$scope.catalogstobeorder[index].selectall;
          console.log($scope.catalogstobeorder[index].title +"  "+ $scope.catalogstobeorder[index].catalogsproducts.length );
          //console.log($scope.catalogstobeorder[index].selectall);
          for(var i=0; i < $scope.catalogstobeorder[index].catalogsproducts.length; i++){
            //console.log($scope.catalogstobeorder[index].catalogsproducts[i])
            
            $scope.catalogstobeorder[index].catalogsproducts[i].is_select = $scope.catalogstobeorder[index].selectall;
          }
        }
        $scope.toggleCheckAll = function(index){
          console.log(index);
          console.log($scope.catalogstobeorder[index].selectall);
          console.log($scope.catalogstobeorder[index].title +"  "+ $scope.catalogstobeorder[index].catalogsproducts.length );
          
          for(var i=0; i < $scope.catalogstobeorder[index].catalogsproducts.length; i++){
              if($scope.catalogstobeorder[index].catalogsproducts[i].is_select == false){
                  $scope.catalogstobeorder[index].selectall = false;
                  return;
              }
          }
          $scope.catalogstobeorder[index].selectall = true;
        }
        ...
        function setProducts(catalogstobeorder, j){
            ...
            Catalog.get($scope.catalog_params).$promise.then(function(result){
                ...
                angular.forEach(result.products, function(product, key) {
                  ...
                  if($scope.cdata.sell_full_catalog == "false" && product.single_piece_price != null){
                    prod.order_price = product.single_piece_price;
                  }
                  else if($scope.receivedcatalogflag == true){
                    prod.order_price = product.final_price;
                  }
                  else{
                    prod.order_price = product.public_price; 
                  }
                  prod.quantity = quantity;
                  //catalogsprice = catalogsprice + product.selling_price * quantity;
                  catalogsprice = catalogsprice + product.order_price * quantity;
                  prod.packingtype = packingtype;
                  if(note){
                    prod.note = note;
                  }
              ...
              /*  for(var i=0; i<(vm.order.products.length); i++){
                ...
                for(var j = 0; j < ($scope.catalogstobeorder.length); j++){
                    for(var i=0; i<($scope.catalogstobeorder[j].catalogsproducts.length); i++){
                        ...
                        //console.log(i)
                        if(product.is_select){
                            vm.items[itemno] = {product:product.id, rate:product.order_price, quantity:product.quantity, packing_type: product.packingtype, note: product.note};
                            itemno++;
                        }
                    }  
                }
                ...
                if(vm.items.length == 0){
                    ...
                    //var jsondata = {order_number:vm.order.order_number, company:$scope.company_id, seller_company:vm.order.supplier, cid:$scope.company_id, items:vm.items, customer_status:"Placed", broker_company:vm.order.agent, note:vm.order.note};
                    ...
                    //var jsondata = {company:$scope.company_id, seller_company:vm.order.supplier, cid:$scope.company_id, items:vm.items, processing_status: 'Draft', broker_company:vm.order.agent, note:vm.order.note};

                    // WB-2838 - Webapp : remove seller name from everywhere - sub task of wishbook (company id = 10) as seller - done by mavaji - 14/09/2018
                    //var jsondata = {company:$scope.company_id, seller_company: 10, cid:$scope.company_id, items:vm.items, processing_status: 'Draft', note:vm.order.note};
                    // jay patel asked to revert again
                    var jsondata = {company:$scope.company_id, seller_company:vm.order.supplier, cid:$scope.company_id, items:vm.items, processing_status: 'Draft', note:vm.order.note};
                    ...
                    function(success){
                     ...
                     /* OrderInvoice.save({"order": $scope.order_id, 'invoiceitem_set': $scope.invoiceitems, 'cid':$scope.company_id },
                          function(success){
                            console.log(success);
                            sharedProperties.setProperty($scope.order_id);
