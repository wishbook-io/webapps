...
(function() {
    ...
    angular
        .module('app.noncatalog')
        .controller('noncatalogController', NoncatalogController);

    NoncatalogController.$inject = ['$http', '$resource', 'v2CategoryEavAttribute', 'EnumGroup', '$stateParams', 'Brand', 'Catalog', 'v2Products', 'CatalogUploadOptions', 'v2ProductsMyDetails', 'v2Category', 'v2BulkUpdateProductSeller', '$timeout', 'SalesOrders', 'toaster', 'ngDialog', '$scope', '$rootScope', 'DTOptionsBuilder', 'DTColumnDefBuilder', 'DTColumnBuilder', 'CheckAuthenticated', '$compile', '$state', 'Upload', '$filter', '$cookies', '$localStorage', 'SweetAlert', 'sharedProperties'];
    function NoncatalogController($http, $resource, v2CategoryEavAttribute, EnumGroup, $stateParams, Brand, Catalog, v2Products, CatalogUploadOptions, v2ProductsMyDetails, v2Category, v2BulkUpdateProductSeller, $timeout, SalesOrders, toaster, ngDialog, $scope, $rootScope, DTOptionsBuilder, DTColumnDefBuilder, DTColumnBuilder, CheckAuthenticated, $compile, $state, Upload, $filter, $cookies, $localStorage, SweetAlert, sharedProperties) {
        ...
        $scope.OpenNoProductsAlert = function (catid, index) {
            vm.errortoaster = {
                type: 'error',
                title: 'Empty Catalog',
        ...
        function imageHtml(data, type, full, meta){
            ...
            return '<div style="text-align: center;"><a ng-click="OpenProductsImages(' + full[0] + ',' + full[10] + ',' + full[0] +')" class="hvr-grow"><img class="loading" src="'+full[4]+'" style="height: 100px; width: 100px;"/></a></div>';
        }
    ...
    $scope.StartStopSelling = function(id)
    {
                ...
                if (product.catalog_with_size == true) {
                    if (product.full_set_only == true) {
                        ...
                        v2BulkUpdateProductSeller.save(params, function (success) {
                            $scope.counter += 1;
                            vm.showMessageIfsuccess('You have successfully started selling all designs of the selected Non-catalogs');
      ...
      vm.dtOptions = DTOptionsBuilder.newOptions()
                      .withOption('ajax', {
                          url: 'api/noncatalogdatatables/?status=enable',
                          type: 'GET',
                      })
                      .withDOM('rtipl')
                    ...
                    vm.dtColumnDefs = [
                        ...
                        DTColumnDefBuilder.newColumnDef(10).withTitle('Designs').withOption('sWidth','5%'),
