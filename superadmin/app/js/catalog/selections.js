...
(function() {
    ...
    SelectionsController.$inject = ['$http', 'Selection', 'BuyerList', 'SupplierList', 'SalesOrders', 'PurchaseOrders', 'Company', '$resource', '$scope', 'DTOptionsBuilder', 'DTColumnDefBuilder', 'DTColumnBuilder', '$compile', '$state', 'CheckAuthenticated', 'toaster', 'SweetAlert', 'ngDialog'];
    function SelectionsController($http, Selection, BuyerList, SupplierList, SalesOrders, PurchaseOrders, Company, $resource,  $scope, DTOptionsBuilder, DTColumnDefBuilder, DTColumnBuilder, $compile, $state, CheckAuthenticated, toaster, SweetAlert, ngDialog) {
      ...
      /*  $scope.ProductsImages = function () {
        ...
        function imageHtml(data, type, full, meta){
          return '<a ng-click="OpenProductsImages('+full[0]+','+full[4]+','+$scope.selection_flag+')"><img class="loading" src="'+full[3]+'" style="width: 100px; height: 100px"/></a>';
        }
        
        function collapseProduct(data, type, full, meta){
            //return '<p title="'+full[2]+'" style="white-space: nowrap; overflow: hidden;text-overflow: ellipsis;  max-width: 200px; cursor: pointer;">'+full[2]+'</p>';
        ...
        function TitleLink(data, type, full, meta){
          if(full[4] == 0){
            ...
            return '<a href="#/app/products-detail/?type=myselection&id='+full[0]+'&name='+full[1]+'">'+full[1]+'</a>';
          }
        }
        ...
        .withLightColumnFilter({
            ...
            2 : { "type" : "text"},
            5 : { "type" : "select", values:[{"value":"Enable","label":"Enable"}, {"value":"Disable","label":"Disable"}]},
        })
        ...
        .withButtons([
            ...
            },
            {
                  ...
                  className: 'green',
                  action: function (e, dt, node, config) {
                    localStorage.removeItem('DataTables_' + 'selection-datatables');
                    $state.go($state.current, {}, {reload: true});
                  }
            },
            //'copy',
        ...
        vm.dtColumnDefs = [
            ...
            DTColumnDefBuilder.newColumnDef(2).withTitle('Product Names').renderWith(collapseProduct),
            DTColumnDefBuilder.newColumnDef(3).withTitle('Image').renderWith(imageHtml).notSortable(),
            DTColumnDefBuilder.newColumnDef(4).withTitle('Designs').notSortable().withOption('sWidth','5%'),
            DTColumnDefBuilder.newColumnDef(5).withTitle('Status').notSortable()
