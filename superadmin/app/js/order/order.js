...
(function() {
    ...
    function OrderController($resource, $http, $httpParamSerializer, toaster, ngDialog, $scope, DTOptionsBuilder, DTColumnDefBuilder, DTColumnBuilder, CheckAuthenticated, $compile, $state,  $filter, SalesOrders, Company, PurchaseOrders, v2PurchaseOrders, Warehouse, OrderInvoice, sharedProperties, $stateParams,Notification,NotificationEntity,NotificationTemplate,ExBee,SweetAlert) {
        ...
        vm.ShipAWB = function(courierid,order_id){
          ...
          SalesOrders.save({'id': $scope.order.id,"order_ids" : $scope.order_ids,"courierid": courierid, 'cid':$scope.company_id,"ship_order_id":$scope.ship_order_id, "order_shipment_id": $scope.order_shipment_id ,"awb_creation": true,
          "combine_multiple" : $scope.combine_multiple,
          "measures":$scope.matrix,
          "sub_resource":"placetoship"
        },
          function(success){
            console.log(success);
        ...
        $scope.PlaceToExpressBee = function(id){
          $scope.order.id = id;
          console.log(id);
          $scope.matrix = {
            'length': '',
            ...
            'awb_no':'',
            'order_id':id,
            'total_shipment_weight':vm.selectedFullJson[id][1]["total_shipment_weight"],
            'shipping_method':vm.selectedFullJson[id][1]["shipping_method"],
            'ship_methods':['Air','Surface']
          ...
          function(success){
            $scope.value_list = success.customer_data;
            console.log($scope.matrix,$scope.value_list);
            $scope.OpenPlaceToExpressBeeDialog();
          });
        }

        $scope.OpenPlaceToExpressBeeDialog = function () {
            ngDialog.open({
                template: 'placeorderonbee',
                scope: $scope,
                className: 'ngdialog-theme-default',
                closeByDocument: false
            });
        ...
        vm.placeOrderonBee = function(){
            if(vm.BeeForm.$valid) {
                $(".modelform4").addClass(progressLoader());
                console.log($scope.matrix);
                ExBee.save({
                  "ShipLength" : $scope.matrix.length,
                  "ShipWidth":$scope.matrix.breadth,
                  "ShipHeight":$scope.matrix.height,
                  "AirWayBillNO" : $scope.matrix.awb_no,
                  "order_id":$scope.matrix.order_id,
                  'total_shipment_weight':$scope.matrix.total_shipment_weight,
                  'ShippingMethod':$scope.matrix.shipping_method,
                  "customer_data":$scope.value_list
                },
                function(success){
                  $(".modelform4").removeClass(progressLoader());
                  ngDialog.close();
        ...
        $scope.openResellerCODLabel = function (order_id) {
            $scope.reseller_cod_label = {};
            ngDialog.open({
                template: 'reseller-cod-label',
                className: 'ngdialog-theme-default',
                scope: $scope,
                closeByDocument: false
            });
            $(".resellercodlabel").addClass(progressLoader());
            SalesOrders.get({'id': order_id, 'cid': $scope.company_id, "expand": "true"}, function(success){
                //$scope.reseller_cod_label = success;
                $scope.reseller_cod_label.facility_type = success.facility_type;
                $scope.reseller_cod_label.display_amount = parseFloat(success.display_amount);
                $scope.reseller_cod_label.shipping_charges = parseFloat(success.shipping_charges);
                $scope.reseller_cod_label.final_display_amount = parseFloat(success.display_amount) + parseFloat(success.shipping_charges);
		 $scope.reseller_cod_label.total_order_amount = parseFloat(success.invoice[0].total_amount);
                $scope.reseller_cod_label.shipping_charges = parseFloat(success.shipping_charges);
                if(success.invoice.length > 0){
                  $scope.reseller_cod_label.paid_amount = parseFloat(success.invoice[0].paid_amount);
                }
                else{
                  $scope.reseller_cod_label.paid_amount = 0;
                }
                $scope.reseller_cod_label.cod_amount = parseFloat(success.display_amount) + parseFloat(success.shipping_charges) - $scope.reseller_cod_label.paid_amount;
                console.log($scope.reseller_cod_label);
                $(".resellercodlabel").removeClass(progressLoader());
            });

        };
        $scope.ChangeToRetail = function(order_id){
          SweetAlert.swal({
                  title: 'Are you sure?',
                  ...
                  confirmButtonText: 'Yes, Change to Retail Order!',
                  ...
                  if (isConfirm) {
                            ...
                            v2PurchaseOrders.get({'id':order_id, 'cid': $scope.company_id, 'sub_resource': 'removereseller'},
                            function(success){
                                ...
                                vm.successtoaster = {
                                    type:  'success',
                                    title: 'Success',
                                    text:  'Order changed to Retail order successfully.'
                                };
                                toaster.pop(vm.successtoaster.type, vm.successtoaster.title, vm.successtoaster.text);
                                $scope.reloadData();
                            });
        ...
        function CartID(data, type, full, meta){
          if(full[1]['cart_id'] != null)
            return full[1]['cart_id'];
          else
            return '';
        }
        ...
        $scope.CancelAndRefund = function(id){
          console.log(id);
        ...
        vm.dtOptions = DTOptionsBuilder.newOptions()
                      ...
                      vm.dtColumnDefs = [
                            ...
                            DTColumnDefBuilder.newColumnDef(1).withTitle('Cart ID').renderWith(CartID),
                            ...
                            DTColumnDefBuilder.newColumnDef(13).withTitle('Order Type'),
                            DTColumnDefBuilder.newColumnDef(14).withTitle('Payment Status'),
                            DTColumnDefBuilder.newColumnDef(15).withTitle('Processing Status'),
                            DTColumnDefBuilder.newColumnDef(16).withTitle('Expected Dispatch Date').notSortable(),
                            DTColumnDefBuilder.newColumnDef(17).withTitle('Sales Order Action').notSortable().withClass('noExport')//.withOption('width', '25%')
                                .renderWith(function(data, type, full, meta) {
                                    //if(full[15].indexOf('Delivered') > -1 || full[15].indexOf('Cancelled') > -1 || full[14].indexOf('Cancelled') > -1)//full[8].indexOf('Dispatched') > -1  ||
                                    // WB-3145 - SuperAdmin: 'Notify' button should remain active even if the order is cancelled
                                    if(full[15].indexOf('Delivered') > -1 )
                                    {
                                        return '&nbsp;';
                                    }
                                    if(full[15].indexOf('Cancelled') > -1 || full[14].indexOf('Cancelled') > -1)
                                    {
                                    ...
                                    if((full[15] == 'Accepted' || full[15] == 'In Progress') && (full[1]['total_pending_quantity'] > 0))
                                    {
                                    ...
                                    if(full[15] == 'Accepted' && full[1]['invoices'].length == 1)
                                    {
                                    ...
                                    if(full[15] == 'Pending' || full[15] == 'Accepted')
                                    {
                                        //htmlbutton += '<div><button type="button" ng-click="OpenCancelOrder('+full[0]+', \'sales\')" class="btn btn-block btn-danger mt-lg">Cancel</button></div>'

                                        // if (full[1]['facility_type'] != 'dropship' &&  full[1]['selling_company_id'] != 10 )
                                        // {
                                          ...
                                          htmlbutton += '<div><button type="button" ng-click="PlaceToExpressBee(' + full[0] + ')" class="btn btn-block btn-primary mt-lg">To ExpressBee</button></div>';
                                    ...
                                    if(full[15] == 'COD Verification Pending')
                                    {
                                       // htmlbutton += '<div><button type="button" ng-click="OpenCancelOrder('+full[0]+', \'sales\')" class="btn btn-block btn-danger mt-lg">Cancel</button></div>'
                                    ...
                                    if(full[14].indexOf('Partially Paid') > -1 && full[11] == 'Yes')
                                    {
                                      htmlbutton += '<div><button type="button" ng-click="openResellerCODLabel('+full[0]+')" class="btn btn-block btn-primary mt-lg">Reseller COD Label</button></div>';
                                    }
                                    if(full[11] == 'Yes' && full[1]['invoices'].length > 0  && full[14].indexOf('Pending') < 0)
                                    {
                                      htmlbutton += '<div><a target="_blank" class="btn btn-primary mt-lg" href="/#/pages/printinvoice/?invoice='+full[1]['invoices'][0]+'">Print Shipping Invoice</a></div>'
                                    }
                                    /*if(full[8] == 'Accepted')
                                    {
                            ...
                            DTColumnDefBuilder.newColumnDef(18).withTitle('Purchase Order Action').notSortable().withClass('noExport')//.withOption('width', '25%')
                                .renderWith(function(data, type, full, meta) {
                                    /*if(full[7] == 'Draft')
                                    {
                                    ...
                                    if(full[15].indexOf('Cancelled') > -1 || full[14].indexOf('Cancelled') > -1)//full[8].indexOf('Dispatched') > -1  ||
                                    {
                                        return '&nbsp;';
                                    }
                                  ...
                                  //  htmlbutton += '<div><button type="button" ng-click="CancelAndRefund('+full[0]+')" class="btn btn-block btn-danger mt-lg">Cancel and Refund</button></div> ';
                                    ...
                                    if(full[14] == 'Pending' || full[15] == 'Pending' || full[15] == 'Accepted')
                                    {
                                       // htmlbutton += '<div><button type="button" ng-click="OpenCancelOrder('+full[0]+', \'purchase\')" class="btn btn-block btn-danger mt-lg">Cancel</button></div> ';
                                    }
                                    if(full[15] == 'COD Verification Pending' && full[11] == 'Yes')
                                    {
                                      htmlbutton += '<div><button type="button" ng-click="ChangeToRetail('+full[0]+')" class="btn btn-block btn-primary mt-lg">Change To Retail</button></div>';
                                    }
                            ...
                            DTColumnDefBuilder.newColumnDef(11).withTitle('Reseller Order').notSortable().withOption('sWidth','20%'),
                            DTColumnDefBuilder.newColumnDef(12).withTitle('Transaction Type').notSortable().withOption('sWidth','20%'),
