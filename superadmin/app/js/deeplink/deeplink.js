...
(function() {
    ...
    function DeeplinkController($resource, $http, $scope, State, Upload, toaster, Brand, Category, EnumGroup, ngDialog, DTOptionsBuilder, DTColumnDefBuilder, DTColumnBuilder, $compile, $state, CheckAuthenticated, resendbuyer, $cookies, $localStorage) {
       ...
       // $scope.brands = Brand.query({showall: true, cid:$scope.company_id, sub_resource:"dropdown"});
        $scope.getBrandList = function(search_param){
            console.log("called:");
            console.log(search_param);
            $scope.brands = Brand.query({showall: true, name: search_param, cid:$scope.company_id, sub_resource:"dropdown"});
        }
        ...
        vm.generateLink = function(){
            ...
            if($scope.product_type !='' && $scope.product_type != undefined){
                $scope.link = $scope.link + '&product_type=' + $scope.product_type;
               // console.log($scope.link);
            }
            ...
            $scope.link = $scope.link.replace("noncatalog","noncatalog,set");
            var check_link = "api/v2/products/dropdown/?view_type=public"+$scope.link;
            // api/v2/products/dropdown/?view_type=public&product_type=noncatalog,set&brand=5565
