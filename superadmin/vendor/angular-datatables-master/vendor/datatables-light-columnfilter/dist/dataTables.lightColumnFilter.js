    ...
    ColumnFilter.filter = {
      text: {
        ...
        dom: function(th){
          ...
          self.elements = $('<input placeholder="&#xF002;" style="font-family:FontAwesome;">', {
            type: 'text'
          }).appendTo(th);
      ...
      select: {
        dom: function(th){
          ...
          select = $('<select>').append('<option value="">All</option>');

          select.addClass(self.options.cssClass);

          $.each(self.options.values, function(ii, value){
            if(self.options.selected == value.value){
                $('<option selected>').val(value.value).text(value.label).appendTo(select);
